...
====================
Maps each interview question to its KEY CONCEPTS that candidates should mention.
Used by the fallback feedback generator to produce accurate, relatable
"What's missing" feedback instead of generic keyword extraction.

Each question maps to 3-5 concise concept phrases that read naturally
when joined in a sentence like:
  "Where I'd push you next is X, Y, Z — these come up as follow-ups..."
"""
...
QUESTION_CONCEPTS = {
    ...
    "What motivates you in your work?": [
        "specific motivators tied to the role (problem-solving, impact)", "connecting personal values to company mission",
    ...
    "When would you optimize for precision vs recall?": [
        "high precision when false positives are costly (spam filtering)", "high recall when false negatives are dangerous (cancer screening)",
    ...
    "Explain the central limit theorem.": [
        ...
        "foundation for statistical inference and hypothesis testing", "larger samples improve the normal approximation"
    ],
    "What is a p-value?": [
    ...
    "Explain convolutional neural networks (CNNs).": [
        "convolution layers detecting spatial features (edges, shapes)", "pooling layers for dimension reduction",
    ...
    "What is autoboxing and unboxing?": [
        "autoboxing: int → Integer automatically", "unboxing: Integer → int automatically",
...
def get_question_concepts(question_text: str) -> list:
    """Look up key concepts for a question. Returns list of concept phrases or empty list."""
    if not question_text:
        return []

    # Try exact match first
    concepts = QUESTION_CONCEPTS.get(question_text)
    if concepts:
        return concepts

    # Try case-insensitive match
    q_lower = question_text.lower().strip()
    for key, val in QUESTION_CONCEPTS.items():
        if key.lower().strip() == q_lower:
            return val

    # Try substring match (question text might be slightly different)
    # Use first 40 chars as a fuzzy key
    q_prefix = q_lower[:40]
    for key, val in QUESTION_CONCEPTS.items():
        if key.lower()[:40] == q_prefix:
            return val
